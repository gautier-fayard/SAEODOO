...
def setup_language(ctx):
    ...
    for code in ALL_LANG:
        ...
        lang = ctx.env["res.lang"].with_context(active_test=False).search([("code", "=", code)])
        ctx.env['base.language.install'].create({'lang_ids': [lang.id]}).lang_install()
        
    # TODO check your date format
    
    ctx.env['res.lang'].search([]).write({'grouping': [3, 0], 'date_format': '%d/%m/%Y'})
